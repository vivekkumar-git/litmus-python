...
def generate_csv(chart_experiment_name_dir, csv_name, csv_config, litmus_env):
    
    csv_filename = chart_experiment_name_dir + '/' + csv_name + '.chartserviceversion.yaml'
...
def generate_chart(csv_parent_path, chart_experiment_name_dir, chart_config, litmus_env):
    chart_filename = chart_experiment_name_dir + '/' + 'experiment.yaml'
    chart_filename_parent_path = csv_parent_path + '/' + 'experiments.yaml'
    ...
    # Generating experiments file in csv_parent_path
    with open(chart_filename_parent_path, "w") as f:
        f.write(output_from_parsed_template)


# generate_rbac creates the rbac for the experiment
def generate_rbac(chart_experiment_name_dir, chart_config, litmus_env):
    rbac_filename = chart_experiment_name_dir + '/' + 'rbac.yaml'
...
def generate_engine(chart_experiment_name_dir, chart_config, litmus_env):
    engine_filename = chart_experiment_name_dir + '/' + 'engine.yaml'
...
def main():
    ...
    entity_name_yaml = config['name']
    category_name_yaml = config['category']
    ...
    entity_name_k8s = entity_name_yaml.replace("_", "-")
    category_name_k8s = category_name_yaml.replace("_", "-")

    # Replace hyphen (-) with underscore (_)
    # in entity_name_yaml and category_name_yaml
    # for file and folder names so that it can be
    # imported as modules
    entity_name = entity_name_yaml.replace("-", "_")
    category_name = category_name_yaml.replace("-", "_")
    ...
    if entity_type == 'chart':
        ...
        # initilise chart directories
        chart_dir = experiment_root_dir + '/charts'
        create_dir(chart_dir)

        chart_experiment_name_dir = chart_dir + '/' + entity_name_k8s
        create_dir(chart_experiment_name_dir)
        ...
        if chartType == "experiment" or chartType == "all":
            ...
            generate_csv(chart_experiment_name_dir, entity_name_k8s, config, env)

            # generate experiment-custom-resource
            generate_chart(chart_dir, chart_experiment_name_dir, config, env)

            # generate experiment specific rbac
            generate_rbac(chart_experiment_name_dir, config, env)

            # generate experiment specific chaos engine
            generate_engine(chart_experiment_name_dir, config, env)
